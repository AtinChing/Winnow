...
let settings: Settings; let adapter: ChatAdapter; let observer: MutationObserver | undefined; let pending = new Set<Element>(); let actions: DomActions | undefined;
...
async function start(): Promise<void> { if (!(await enabled())) { observer?.disconnect(); actions?.destroy(); actions = undefined; return; } actions ??= new DomActions(); adapter = getAdapter(location.hostname); const find = () => { const container = adapter.findContainer(); if (container) attach(container); else window.setTimeout(find, 1000); }; find(); }

...
function process(): void { const nodes = [...pending]; pending.clear(); for (const node of nodes) { actions.reset(node); const ctx = adapter.extract(node); if (!ctx.text) continue; const result = pipeline.check(ctx, settings); if (result.blocked) actions.block(node, result.reason ?? 'filtered'); } }

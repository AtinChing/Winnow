export interface ToxicityClassifier { ready(): Promise<void>; classify(text: string): Promise<{ toxic: boolean; score: number }>; }
...
export class StubToxicityClassifier implements ToxicityClassifier {
  async ready(): Promise<void> { return; }
  async classify(_text: string): Promise<{ toxic: boolean; score: number }> { return { toxic: false, score: 0 }; }
}
